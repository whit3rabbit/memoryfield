...
on stdin, nothing else to install). `mf setup install --harness claude
--hooks` writes them. By hand, add to `.claude/settings.json` (or
`settings.local.json`) at the project root, with `--field` naming the
field directory relative to that root (drop it when the root is the
field):
...
Claude Code runs hooks with cwd set to the project root, so without
`--field` a field kept in a subdirectory never fires.
...
are enough (`mf setup install --instructions` writes them in a fenced
block; `--field` names the field relative to the project root):
...
[DIR] [--model NAME]` creates the index, and the model is fixed per
field (`mf model list` for the registry, docs/models.md in the mf repo
for which to pick). `mf pack [DIR]` writes
